...
export async function TestChores() {
                            ...
                            if (test_loadChoreData.name == "N_UT" && test_loadChoreData.frequency == "FR_UT" && test_loadChoreData.assignees == "ASS_UT" && test_loadChoreData.details == "DET_UT" && test_loadChoreData.completed == "CMP_UT") {
                                ...
                                choreFunctions.updateChore(U_TESTER_CHORE, C_TEST_ID).then(() => {
                                    db.ref(`/suites/test123/chores/${C_TEST_ID}`).once('value', snapshot => {
                                        let c_data = snapshot.val();
                                        console.log(c_data);
                                        console.log("here");
                                        if (c_data.name == "U_N_UT" && c_data.frequency == "U_FR_UT" && c_data.assignees == "U_ASS_UT" && c_data.details == "U_DET_UT" && c_data.completed == "U_CMP_UT") {
                                ...
                                db.ref(`/users/${U_TEST_UID}/suiteID`).once('value', snapshot => {
                                    ...
                                    if(data == U_TEST_SID_NEW) {
                                        ...
                                        userFunctions.updateUserDetails(U_TEST_UID, U_TEST_NAME_NEW, U_TEST_PRONOUNS_NEW).then(() => {
                                            db.ref(`/users/${U_TEST_UID}`).once('value', snapshot => {
                                                let u2_data = snapshot.val();
                                                if(u2_data.uid == U_TEST_UID && u2_data.name == U_TEST_NAME_NEW && u2_data.pronouns == U_TEST_PRONOUNS_NEW && u2_data.suiteID == U_TEST_SID_NEW) {
                                                    console.log("USERS/updateUserDetails: PASSED");

                                                    userFunctions.getUserData(U_TEST_UID).then((ug_data) => {
                                                        if(ug_data.uid == U_TEST_UID && ug_data.name == U_TEST_NAME_NEW && ug_data.pronouns == U_TEST_PRONOUNS_NEW && ug_data.suiteID == U_TEST_SID_NEW) {
